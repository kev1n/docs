...
On [deadline date], `graph-node` will support 100% coverage of the [GraphQL Validations specification](https://spec.graphql.org/June2018/#sec-Validation).
...
```bash
npx @graphql-validate/cli -s https://api-next.thegraph.com/subgraphs/name/$GITHUB_USER/$SUBGRAPH_SLUG -o *.graphql
```
...
The `[@graphql-validate/cli](https://github.com/saihaj/graphql-validate)` CLI tool will output the GraphQL operations errors as follows:

![Untitled](GraphQL%20Validations%20migration%20guide%20ba6225eb33e54d0e88907d7abaa50a5e/Untitled.png)
...
- `https://api-next.thegraph.com/subgraphs/name/your/subgraph`
...
query {
  user {
    id { # Invalid, because "id" is of type ID and have not sub-fields
...
# This also might happen when incorrect variable is defined:
